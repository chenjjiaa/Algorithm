...
public class A28_KMP_字符串匹配 {
    ...
    /**
     * 在整个判断过程中，str1 的 index移动是一直向右的，
     * 而 str2的移动是可以向左移动的，以此来加速判断
     */
    public static int getIndexOf(String str1, String str2) {
        ...
        int i1 = 0; // 主串的下标索引，i1 是不一定要遍历完的
        int i2 = 0; // 子串的下标索引
        while (i1 < chars1.length && i2 < chars2.length) {
            if (chars1[i1] == chars2[i2]) { // 若两个字符相同
                i1++;
                i2++;
            } else if (nextArray[i2] == -1) { // 跳到不能跳了
                i1++; // 主串换到下一个字符，和子串继续对比
            } else { // 能跳则跳
                i2 = nextArray[i2];
            }
        }
    ...
    public static int[] getNextArray(char[] chars) {
        ...
        nextArray[0] = -1;
        nextArray[1] = 0;
        int cn = 0; // 要拿哪个字符和 i-1 的字符比，到时候是对这个变量进行向前移动，变量记录了两个信息：①i-1的最长前缀 ②要移动得到的索引下标
